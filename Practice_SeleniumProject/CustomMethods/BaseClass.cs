...
namespace Practice_SeleniumProject.CustomMethods
{
    public class BaseClass
    {
        ...
        public void DragAndDropItem(IWebDriver driver, IWebElement sourceElement, IWebElement destinationElement)
        {
            Actions action = new Actions(driver);
            action.ClickAndHold(sourceElement).MoveToElement(destinationElement).Release().Build().Perform();
        }

        public void ActionClick(IWebDriver driver, IWebElement Element)
        {
            Actions action = new Actions(driver);
